...
def run(args):
    ...
    load_path = '/path/to/your/pretrained/model.pth'
    ...
    # Dataset
    train_loader, val_loader = make_target_loader(batch_size=args.batch_size, patch_flag=False)
    ...
    for epoch in range(args.epochs):
        # Train
        train_loss = train(args, epoch, model_encoder, model_decoder, criterion, optimizer, train_loader, logger)

        # Validation
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            val_loss = val(args, epoch, model_encoder, model_decoder, criterion, val_loader, logger)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--result', default='noise_ssl', type=str, help='path to results')
