...
class TargetDataset(data.Dataset):
    def __init__(self, noise_factor=0.25, patch_ratio=0.05, patch_cnt=20,
                 csv_path='/mnt/hgh/JaPyGuri-AI/dataset/target_unlabeled/HandInfo.csv'):
        super().__init__()
        ...
        dir_name = "/mnt/hgh/JaPyGuri-AI/dataset/target_unlabeled/Hands/Hands/"
...
def make_target_loader(batch_size=8, noise_factor=0.25, patch_ratio=0.05, patch_cnt=20):
    dataset = TargetDataset(noise_factor, patch_ratio, patch_cnt)
    total_size = len(dataset)
...
if __name__ == "__main__":
    ...
    for inputs, targets in val:
        print(f'noisy(input) - shape : {inputs.shape}, max : {torch.max(inputs)}, min : {torch.min(inputs)}')
        print(f'origin(target) - shape : {targets.shape}, max : {torch.max(targets)}, min : {torch.min(targets)}')
